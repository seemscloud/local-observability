...
- `config/cockpit/cockpit.conf` configures both trusted site domains, forwarded HTTPS, and the passwordless local session.
- `ops/cockpit-host-bridge.sh` enters the local host namespaces and starts the host-installed Cockpit bridge as root.
