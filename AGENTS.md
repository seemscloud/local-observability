...
- `config/grafana/dashboards/compute-host.json` defines the comprehensive `Compute - Host` dashboard with a dynamic multi-host selector and metrics verified on both NUCs.
- `config/grafana/dashboards/compute-banana-pi-r4.json` defines the comprehensive `Network - Banana Pi R4` dashboard with a dynamic multi-router selector and metrics verified on both OpenWrt routers.
