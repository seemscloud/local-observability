...
Each NUC runs its own Portainer CE instance against the local Docker socket. Portainer data is retained in `observability-portainer-data`; its internal HTTPS port `9443` is not published. Create the ignored `.portainer/admin-password`, `.portainer/encryption-key`, and `.portainer/agent-secret` files with mode `0600` before first deployment. The shared agent secret protects Portainer Agent on private LAN port `9001`. The Compose bootstrap registers `szew-nuc` in Faszyn and `faszyn-nuc` in Szew, so both UIs contain the local `primary` Docker environment and the opposite NUC.
