...
The root-level **Compute - Host** and **Network - Banana Pi R4** dashboards cover the available CPU, memory, filesystem, disk, network, connection tracking, thermal, hardware, and exporter-health metrics. Every metric is shown as a historical time-series chart in a compact three-column layout with no current-value stat cards. Their **Host** and **Router** filters are populated dynamically from Prometheus and support one, multiple, or all monitored nodes. Queries were validated against both Faszyn and Szew targets; metrics absent or invalid on either site are not rendered as empty panels.
